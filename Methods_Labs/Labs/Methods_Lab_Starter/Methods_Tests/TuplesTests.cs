...
namespace Methods_Tests
{
    public class TuplesTests
    {
        ...
        [Test]
        public void GivenANumber_squareCubeRoot_ReturnsExpectedValue()
        {
            int input = 4;
            int expectedSqaure = 16;
            int expectedCube = 64;
            double expectedRoot = 2;

            var result = Methods.squareCubeRoot(input);

            Assert.That(result.square, Is.EqualTo(expectedSqaure));
